...
struct page
{
	...
	union
	{
		...
		// 파일에 기반하고 있지 않은(맵핑된 파일이 없는) 페이지
		// 커널로부터 프로세스에게 할당된 일반적인(비 디스크 기반) 메모리 페이지
		struct anon_page anon;

		// 파일으로부터 매핑된 페이지 ( 디스크에서 읽어온다. )
...
struct frame
{
	void *kva;					 // 커널 가상 주소: 물리메모리 프레임이랑 일대š로 매핑되어 있는 가상 주소
...
struct page_operations
{
	// 각 페이지에 취할 수 있는 여러가지 액션들이 function pointer 형태로 저장
	bool (*swap_in)(struct page *, void *); // 디스크 swap area -> 물리 frame
	bool (*swap_out)(struct page *);		// 물리 frame -> 디스크 swap area

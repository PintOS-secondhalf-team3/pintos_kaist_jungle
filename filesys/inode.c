...
{ // file growth 파트에서 수정해야함
	// 원래는 인자로 length(파일의크기)를 받았는데 이젠 length는 0으로 들어옴
	// 즉, inode_create()때는 inode만 가지고 있는 file하나만 생성되는 것
	// inode_write_at()에서 데이터를 받는대로 EOF를 늘려야된다.
					 ...
					 off_t offset)
{
	// inode_write_at()에서 데이터를 받는대로 EOF를 늘려야된다.
	// 인자는 어떻게 들어올까?
	// 신생 파일의 경우에는 offset은 0으로 들어오고, size는 실제 file length만큼 들어올 것이다.
